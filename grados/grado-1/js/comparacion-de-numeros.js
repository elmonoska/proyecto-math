...
btnConsultaComparacion.addEventListener('click', () => {
    const tbodyTableComparacion2Numeros = document.querySelector('#comparacion2Numeros')
    ...
    if (isNaN(inputNumero1.value) || isNaN(inputNumero2.value) || inputNumero1.value == '' || inputNumero2.value == '') {
        msgErrorComparacion.textContent = 'Debes introducir solamente numeros.'
    } else {
        msgErrorComparacion.textContent = ''
        tbodyTableComparacion2Numeros.children[1]
        .children[0]
        .children[1]
        .children[0].textContent = `${agregarSignoComparacion(parseInt(inputNumero1.value), parseInt(inputNumero2.value))}`
    }
